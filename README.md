...
- `index.qmd` — 서문 / 책 사용법
- `weekNN-theory.qmd` — 주차별 **이론** (개념 + Mermaid 그림, 한글 8,000자 이상)
- `weekNN-quiz.qmd` — 주차별 **연습문제** (4지선다 20문항 + 정답·해설)
- `weekNN-lab.qmd` — 주차별 **코딩실습** (예제 코드 30분 + 학생 실습 과제 30분)
- `styles.scss` — 한글 웹 스타일

> **참고문헌은 배포본에 공개하지 않습니다.** 각 주차의 참고문헌 목록·통합 참고문헌 페이지·BibTeX(`references.bib`)와 본문 인라인 인용은 배포본에서 제거되어 있으며, 원본은 저장소 바깥 `../참고문헌_비공개/` 폴더에 주차별로 보관합니다(GitHub·사이트에 올라가지 않음).
