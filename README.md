...
> - **연습문제**: 선택지를 체크박스처럼 클릭해 답을 고른 뒤(문항당 하나), 맨 아래 **정답확인** 버튼을 누르면 전체 정답(✓)·내가 고른 오답(✗)과 해설이 한꺼번에 표시되고 점수가 요약됩니다. 정답은 채점 전에는 노출되지 않습니다. (구현: `interactive.html` + `styles.scss`의 `.quiz-opt`·`.quiz-actions`)
